...
chisq_test_result <- chisq.test(table(fire_rescue_clean$age_band, fire_rescue_clean$extrication))
...
reg_model_summary <- bind_rows(
  tidy(model_multi, conf.int = TRUE, exponentiate = TRUE) %>% mutate(model = "Multinomial GLM"),
  tidy(model_multi2, conf.int = TRUE, exponentiate = TRUE) %>% mutate(model = "Updated Multinomial GLM"),
